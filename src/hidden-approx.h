 ...
 *
 * RCSId: $Id: hidden-approx.h 1 2013-03-28 13:54:24Z hanne $
 *
 */
/*!
...
typedef struct
{
    ...
    */
    ...
    /*!
      \brief  Expand around the current value (see #GMRFLib_MODEOPTION_CURRENT) or
      the mode (see #GMRFLib_MODEOPTION_MODE).
    */    
    int modeoption;

    /*!
      \brief If \c TRUE, use the Gaussian approximation instead of computing the non-Gaussian
      approximation. This choice will override all other options.
    */    
    int gaussapprox;
    ...
    */
    ...
    /* 
       ...
       if we control the remap, and only sample from n-1 .... stop_idx, then we sample from a
       marginal of the full conditional. this have useful applications. these options are intented
       used togther, but does not. note that `remap' is default NULL and stop_idx is default 0, ie
       sample from the full conditional.
    */
    int *remap;					  /* force to use this remap n  */
    int stop_idx;				  /* sample/evalutate the marginal x_from n-1 to
						     stop_idx, in the permuted world. */
    GDens **marginal_density;			  /* if not NULL, then store the LAST marginal as
						     (*marginal) instead of FREE'ing it. */
    double *marginal_mean, *marginal_stdev;	  /* the density stored in `marginal' is adjusted
						     for the marginal mean. hence to evaluate the
						     density in 'x' then marginal density must be
						     evaluated at 'x-marginal_mean'. the
						     marginal_stdev is returned [optionally] for
						     convenice. */
}
...
typedef struct
{
    ...
    */    
    ...
    /*!
      \brief Internal use only
    */    
    char *ran_approx_state;

    /*!
      \brief Internal use only
    */    
    char *ran_current_state;
    ...
    */
    ...
    /*!
      \brief Parameters which control the approximation
      \sa GMRFLib_default_hidden_par
    */    
...
int GMRFLib_init_problem_hidden(
    GMRFLib_hidden_problem_tp **hidden_problem,
...
int GMRFLib_init_problem_hidden_store(
    GMRFLib_hidden_problem_tp **hidden_problem,
    double *x, double *b, double *c, double *mean,
    GMRFLib_graph_tp *graph, GMRFLib_Qfunc_tp *Qfunc, char *Qfunc_args,
    char *fixed_value,
