...
/*!
  ...
  If an error occurs running the routines from \em GMRFLib, an error message 
  will be issued stating the type of error, and the function, the file and 
  the line at which the error has occurred.  By default, the program is core 
  dumped and aborted at the occurrence of an error, using the function 
  \c GMRFLib_error_handler().  This side effect can be overridden by the 
  user, re-defining the error handling function as described in 
  \c GMRFLib_set_error_handler().
*/
...
static const char RCSId[] = "$Id: error-handler.c 1 2013-03-28 13:54:24Z hanne $";
...
/*!
  ...
  4 :    Invalid argument \n
  5 :    Graph is invalid \n
  6 :    Error while reordering the graph \n
  7 :    Error while reading graph from file \n
...
#define NMSG 21
    ...
    { "No error, please ignore",
      ...
      "Graph is invalid",   
      "Error while reordering the graph", 
      "Error while reading graph from file",   
      "Error opening file",   
      ...
      "Error occured in the dcdf-library",
...
/*!
  ...
  \remarks This is the default error handling function integrated in the library routines.
  It is called by the function \c GMRFLib_handle_error(). When an error occurs, the 
  function prints an error message of the form
  \verbinclude example_doxygen_error_1.txt
  and the program is aborted, with a core dump. In the example above, the error message 
  states that the command line arguments of the main program on the file \c test-graph.c 
  is missing. To avoid aborting the program, a new error handling function can be defined,
  returning the error code without aborting. To change to another error handling function, 
  use \c GMRFLib_set_error_handler().
			  ...
			  const char *id, int errorno, const char *msg)
{
    ...
    if (function)  fprintf(stderr, "\tFunction  : %s\n", function);
    if (file)      fprintf(stderr, "\tFile      : %s\n", file); 
    if (1)         fprintf(stderr, "\tLine      : %1d\n", line);
    if (id)        fprintf(stderr, "\tRCSId     : %s\n", id); 
    fprintf(stderr, "\n");

    if (errorno != GMRFLib_SUCCESS) abort();	  /* no reason to abort if ok */
 ...
 */
int GMRFLib_handle_error(const char *file, const char *function, int line,
			 const char *id, int errorno, const char *msg)
{
    /* 
       this function handle an error, and is the one called from the library through the macro
       'GMRFLib_ERROR'
     */
    if (handler)
...
/*!
  ...
  \sa GMRFLib_error_handler_tp, GMRFLib_error_handler.
 */
int GMRFLib_set_error_handler(GMRFLib_error_handler_tp *new_error_handler)
{
    /* 
       set new error-handler. if the new error-handler is NULL, then the default error-handler is
       used.
     */
