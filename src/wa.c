...
static const char RCSId[] = "$Id: wa.c 1 2013-03-28 13:54:24Z hanne $";
			    ...
			    GMRFLib_graph_tp *wagraph, GMRFLib_Qfunc_tp *wafunc, char *wafunc_arg)
{
    ...
    *wa_problem              = Calloc(1, GMRFLib_wa_problem_tp); MEMCHK(*wa_problem);
    (*wa_problem)->graph     = graph;
    (*wa_problem)->Qfunc     = GMRFLib_waQfunc;
    (*wa_problem)->Qfunc_arg = (char *)wa_arg;

    /* 
       prune graph. note that waQgraph is the original unpruned graph which is needed for correct
       computing of Q_ij after pruning.
     */
    GMRFLib_prune_graph(&ngraph, (*wa_problem)->graph,
			(*wa_problem)->Qfunc, (*wa_problem)->Qfunc_arg);
...
*/
double GMRFLib_waQfunc(int node, int nnode, char *arg)
{
    int j, jj, k, count, idx;
    double val, tmp, *ptr;
    GMRFLib_waQfunc_arg_tp *args;
	...
	if (node==nnode) 
	{
		...
		*/
		for(j=0, count=0;j<args->waQgraph->nnbs[node];j++)
		{
		    ...
		    if (GMRFLib_is_neighb(k, node, args->wagraph)) count++;
		}
		args->neigb_info[idx] = Calloc(1, GMRFLib_node_list_tp);
		if (count) args->neigb_info[idx]->node_list = Calloc(count, int);
	...
	else
	{
		...
		*/
		...
		for(j=0, count=0;j<args->waQgraph->nnbs[node];j++)
		{
		    k = args->waQgraph->nbs[node][j];
		    if (GMRFLib_is_neighb(k, node,  args->wagraph) &&
			GMRFLib_is_neighb(k, nnode, args->wagraph)) count++;
		}
		if (count) args->neigb_info[idx]->node_list = Calloc(count, int);
		args->neigb_info[idx]->n_nodes = count;
		for(j=0, jj=0;j<args->waQgraph->nnbs[node];j++)
...
/*!
  ...
  and \em j and a \em character -pointer referring to additional arguments to the function. If \f$
  w_{ij,s}=0 \f$ initially, it is required to be kept unchanged.

  \param[in] wafunc_arg An array with length \a n_wa of \em character-pointers holding the address
  ...
  \param[in] Qfunc_arg An array with length \a n_g of \em character-pointers holding the address of
			     ...
			     int n_g,  GMRFLib_graph_tp **graph,   GMRFLib_Qfunc_tp **Qfunc,  char **Qfunc_arg)
{
    /* 
       ...
       this routine setup a sequence of wa_problems and ordinary graphs/Qfuncs and return the graph
       and the Qfunction for the sum.
       
    */

    int k;
    GMRFLib_nwa_problem_tp **nwa;
	...
	*nwa_problem = NULL;
    ...
    nwa        = Calloc(n_wa + n_g + 1, GMRFLib_nwa_problem_tp *); MEMCHK(nwa); /* yes! use a NULL-terminated list */
    nwa_graphs = Calloc(n_wa + n_g    , GMRFLib_graph_tp *); MEMCHK(nwa_graphs);
    
    for(k=0;k<n_wa;k++)				  /* do the wa_problems */
    {
	GMRFLib_wa_problem_tp *wa;
    ...
    {
	nwa[k+n_wa]             = Calloc(1, GMRFLib_nwa_problem_tp); MEMCHK(nwa[k+n_wa]);
    ...
    *nwa_problem = Calloc(1, GMRFLib_nwa_problem_tp); MEMCHK(*nwa_problem);

    GMRFLib_union_graph(&((*nwa_problem)->graph), nwa_graphs, n_wa+n_g);
    FREE(nwa_graphs);

    (*nwa_problem)->Qfunc      = GMRFLib_nwaQfunc;
    (*nwa_problem)->Qfunc_arg  = (char *) nwa;
...
int GMRFLib_free_nwa_problem(GMRFLib_nwa_problem_tp *nwa_problem)
{
    ...
    if (!nwa_problem) return GMRFLib_SUCCESS;
...
double GMRFLib_nwaQfunc(int node, int nnode, char *arg)
{
    int k, equal;
    double val = 0.0;
    GMRFLib_nwa_problem_tp **nwa;
...
/*! \page ex_wa A worked out example smoothing a time-series data, using the wa_problem feature
  ...
  In this example, we will illustrate the \em wa_problem feature, which is quite useful in
  situations like the following.  Consider the time-series data <em>\b y</em> in Fig..., which we
  assume to be observed according to the model \f[ y_i = x_i + \epsilon_i,\f] where <em>\b x</em> is
  a hidden smooth curve and \f$ \bf\epsilon \f$ is \em iid Gaussian noise with zero mean and
  variance \f$ 0.25^2 \f$ .

  As a prior, or a smoothing-prior, for <em>\b x</em>, we will consider two models: a first order
  model \f[\pi(\bf{x}) \propto \exp\left(-\frac{h}{2} \sum_{i=2}^n (x_i-x_{i-1})^2\right)
  \hspace{2cm} (wa-1) \f] and a second order model \f[ \pi(\bf{x}) \propto \exp\left(-\frac{h}{2}
  \sum_{i=3}^n (x_i-2x_{i-1}+x_{i-2})^2\right). \hspace{2cm} (wa-2) \f] The quantities of interest
  are the posterior mean for <em>\b x|y</em> and samples from the posterior \f[ \pi(\bf{x}|\bf{y})
  \propto \exp\left(-\frac{h}{2} \sum_{i=2}^n (x_i-x_{i-1})^2+ \sum_{i=1}^n
  (y_ix_i/\sigma^{2}+x_i^2/\sigma^{2}) \right). \hspace{2cm} (wa-3) \f] A similar expression can be
  obtained for the second order model.

  To implement Equation <b>(wa-3)</b>, we observe that the prior is of the \em wa_problem form
  <b>(GMRF-8)</b> in \ref specification, for which we use the function \c GMRFLib_init_wa_problem()
  to convert the prior into the standard <em>\b x'Qx</em>-form. The data will define the <em>\b
  b</em> and <em>\b c</em> vector in \c GMRFLib_init_problem().
