...
static const char RCSId[] = "$Id: hidden-approx.c 1 2013-03-28 13:54:24Z hanne $";
...
{
    *hidden_par = Calloc(1, GMRFLib_hidden_param_tp); MEMCHK(*hidden_par);
    ...
    (*hidden_par)->nantithetic = 1;		  /* # of antithetic samples is GMRFLib_NUM_ANTITHETIC (=4)
						     times this: +-  x (u,1-u)scale */
    ...
    (*hidden_par)->step_len    = 1.0e-4;	  /* step-length for Taylor expantion */
...
int GMRFLib_loglFunc_wrapper(double *logll, double *x, int m, int idx, double *x_vec, char *arg)
{
    /* 
	...
	FREE(ff); FREE(xx);
	len = m;
	ff  = Calloc(len, double); MEMCHK(ff);
	xx  = Calloc(len, double); MEMCHK(xx);
    }
    
    args = (GMRFLib_logl_arg_tp *)arg;
    mean = args->sub_mean[idx];
    for(i=0;i<m;i++) xx[i] = x[i] + mean;
    ...
    for(i=0;i<m;i++) logll[i] = ff[i] - (a+(b+c*xx[i])*xx[i]);

    if (0)					  /* FIXME */
	for(i=0;i<m;i++) logll[i] = MIN(logll[i], 0.0);
...
/*!
  ...
  \param[in] loglFunc_arg A \em character pointer holding the address of a variable
				       ...
				       GMRFLib_store_tp *store)
{
    ...
    if (store) FIXME("store-functionality is not yet implemented.");

    if (!fixed_value)
    {
	fixed_value = Calloc(graph->n, char); MEMCHK(fixed_value);
	free_fixed_value = 1;
    }
    if (!x)
    {
    ...
    (*hidden_problem) = Calloc(1, GMRFLib_hidden_problem_tp); MEMCHK((*hidden_problem));
    if (hidden_par)
    {
	/* 
	   copy remap (if !NULL) later, as i need to know the size of sub_graph
	*/
	(*hidden_problem)->hidden_par = Calloc(1, GMRFLib_hidden_param_tp); MEMCHK((*hidden_problem)->hidden_par);
    ...
    */
    mode = Calloc(graph->n, double); MEMCHK(mode);
    memcpy(mode, x, graph->n*sizeof(double));
    if ((*hidden_problem)->hidden_par->modeoption == GMRFLib_MODEOPTION_MODE)
	GMRFLib_optimize(mode, b, c, mean, graph, Qfunc, Qfunc_args, fixed_value, NULL,
			 d, loglFunc, loglFunc_arg, optpar);

    /* 
       convert the variables so we get rid of the fixed_values and then write the function in the
       canonical form
    ...
    */
    GMRFLib_compute_subgraph(&tmp_graph, graph, fixed_value);
    sub_n = tmp_graph->n;
	...
	if (free_fixed_value) FREE(fixed_value);
	if (free_x) FREE(x);
    ...
    */
	...
	tmp_fact.bandwidth = GMRFLib_compute_bandwidth(tmp_graph, tmp_fact.remap);

	(*hidden_problem)->hidden_par->remap = Calloc(sub_n, int);
	memcpy((*hidden_problem)->hidden_par->remap, hidden_par->remap, sub_n*sizeof(int));

	if (0) printf("use users remap with bandw %d\n", tmp_fact.bandwidth);
    }
    else
    {
	GMRFLib_compute_reordering(&tmp_fact, tmp_graph);
    }
    ...
    */
    ...
    for(i=0;i<sub_n;i++) (*hidden_problem)->sub_sm_fact.remap[i] = i;
    inv_remap = Calloc(sub_n, int); MEMCHK(inv_remap);
    (*hidden_problem)->map = Calloc(sub_n, int); MEMCHK((*hidden_problem)->map);
    for(i=0;i<sub_n;i++) inv_remap[tmp_fact.remap[i]] = i;
    for(i=0;i<sub_n;i++) (*hidden_problem)->map[i] = tmp_graph->mothergraph_idx[inv_remap[i]];
    GMRFLib_remap_graph(&((*hidden_problem)->sub_graph), tmp_graph, tmp_fact.remap);
    ...
    FREE(inv_remap);GMRFLib_free_graph(tmp_graph); GMRFLib_free_reordering(&tmp_fact);
    
    /* 
       setup space & misc. sub_mean is also sub_mode as we're generating a gaussian approximation
       centered at the mode
    */
    (*hidden_problem)->sample      = Calloc(graph->n, double); MEMCHK((*hidden_problem)->sample);
    (*hidden_problem)->sample_star = Calloc(graph->n, double); MEMCHK((*hidden_problem)->sample_star);
    (*hidden_problem)->sub_sample  = Calloc(graph->n, double); MEMCHK((*hidden_problem)->sub_sample);
    (*hidden_problem)->sub_mean    = Calloc(sub_n, double); MEMCHK((*hidden_problem)->sub_mean);
    (*hidden_problem)->sub_b       = Calloc(sub_n, double); MEMCHK((*hidden_problem)->sub_b);
    (*hidden_problem)->sub_d       = Calloc(sub_n, double); MEMCHK((*hidden_problem)->sub_d);
    ...
    for(i=0;i<sub_n;i++)        (*hidden_problem)->sub_mean[i] = mode[(*hidden_problem)->map[i]];
    if (b) for(i=0;i<sub_n;i++) (*hidden_problem)->sub_b[i] = b[(*hidden_problem)->map[i]];
    if (d) for(i=0;i<sub_n;i++) (*hidden_problem)->sub_d[i] = d[(*hidden_problem)->map[i]];
    if (c) for(i=0;i<sub_n;i++) cc[i] = c[(*hidden_problem)->map[i]];

    (*hidden_problem)->x_vec = Calloc(graph->n, double); MEMCHK((*hidden_problem)->x_vec);
    ...
    */
    ...
    (*hidden_problem)->sub_Qfunc_arg                  = Calloc(1, GMRFLib_Qfunc_arg_tp); MEMCHK((*hidden_problem)->sub_Qfunc_arg);
    ...
    GMRFLib_ASSERT(((*hidden_problem)->hidden_par->neightype == GMRFLib_NEIGHTYPE_LINEAR  ||
		    (*hidden_problem)->hidden_par->neightype == GMRFLib_NEIGHTYPE_GRAPH),
		   GMRFLib_EPARAMETER);
    GMRFLib_ASSERT((*hidden_problem)->hidden_par->cmeanmode == GMRFLib_COND_MEAN ||
		   (*hidden_problem)->hidden_par->cmeanmode == GMRFLib_COND_MODE,
		   GMRFLib_EPARAMETER);
    GMRFLib_ASSERT((*hidden_problem)->hidden_par->norder == 1 || (*hidden_problem)->hidden_par->norder == 2,
		   GMRFLib_EPARAMETER);
	...
	for(j=0;j<graph->nnbs[node];j++)  /* then over all neighbors */
	{
		...
		*/
		...
		/* 
		   nnone is not fixed
		*/
		if (mean) (*hidden_problem)->sub_b[i] += qvalue*mean[nnode];
	    }
	}
    }

    /* 
       setup wrapper for the loglFunc, this involves BOTH the 'index' and correction for the
       quadratic term
    */
    (*hidden_problem)->loglFunc     = GMRFLib_loglFunc_wrapper;
    args                            = Calloc(1, GMRFLib_logl_arg_tp); MEMCHK(args);
    args->loglFunc                  = loglFunc;
    args->loglFunc_arg              = loglFunc_arg;
    args->map                       = (*hidden_problem)->map;
    args->sub_mean                  = (*hidden_problem)->sub_mean;
    args->acoof                     = Calloc(sub_n, double); MEMCHK(args->acoof);
    args->bcoof                     = Calloc(sub_n, double); MEMCHK(args->bcoof);
    args->ccoof                     = Calloc(sub_n, double); MEMCHK(args->ccoof);
    (*hidden_problem)->loglFunc_arg = (char *)args;


    /* 
       add term from the loglikelihoodterm
    */
    for(i=0;i<sub_n;i++) (*hidden_problem)->x_vec[(*hidden_problem)->map[i]] = mode[i];
    for(i=0;i<sub_n;i++)
    {
	if ((*hidden_problem)->sub_d[i])
	{
	    ...
	    args->ccoof[i] = MAX(0.0, args->ccoof[i]);
	}
	else
    ...
    /* 
       now compute the Cholesky-factorisation. I solve one additonal time to get the mean, even
       though I know it already as the mode [cost is nothing...]
    */


    GMRFLib_build_sparse_matrix(&((*hidden_problem)->sub_sm_fact),  (*hidden_problem)->sub_Qfunc, 
				(char*)((*hidden_problem)->sub_Qfunc_arg), (*hidden_problem)->sub_graph);
    GMRFLib_factorise_sparse_matrix(&((*hidden_problem)->sub_sm_fact), (*hidden_problem)->sub_graph);

    memcpy((*hidden_problem)->sub_mean, (*hidden_problem)->sub_b, sub_n*sizeof(double));
    GMRFLib_solve_llt_sparse_matrix((*hidden_problem)->sub_mean, &((*hidden_problem)->sub_sm_fact), (*hidden_problem)->sub_graph);
    ...
    */
	...
	GMRFLib_make_linear_graph(&((*hidden_problem)->hidden_par->ngraph),
				  (*hidden_problem)->sub_graph->n,
				  (*hidden_problem)->hidden_par->neighpar, 0);
	break;
    case GMRFLib_NEIGHTYPE_GRAPH:
	GMRFLib_nfold_graph(&((*hidden_problem)->hidden_par->ngraph),
			    (*hidden_problem)->sub_graph,
			    (*hidden_problem)->hidden_par->neighpar);
	break;
    }

    FREE(mode); if (free_fixed_value) FREE(fixed_value); if (free_x) FREE(x);
...
int GMRFLib_free_hidden(GMRFLib_hidden_problem_tp *hidden_problem)
{
    ...
    */
    if (0) if (h->hidden_par->marginal_density) GDensFree(*(h->hidden_par->marginal_density));
...
/*!
  ...
  the problem specification as defined by a call to \c GMRFLib_init_problem_hidden().
  At output, a sample of the HGMRF has been generated, and is stored
  in the \em sample member of the data structure \a hidden_problem.
  Also, the log-density of the sample is computed, and stored in the
  \a sub_logdens member. If \a hidden_problem = \c NULL, nothing is done,
  and the routine returns 0.
  
  \remark To sample one realization from a HGMRF <em>\b x|y</em>, first invoke
  \c GMRFLib_init_problem_hidden() generating the problem specification by initializing 
  a \c GMRFLib_hidden_problem_tp -object \a hidden_problem, and then call 
  \c GMRFLib_hidden_sample() using \a hidden_problem as an argument. Repeated 
  samples are generated by repeated calls to \c GMRFLib_hidden_sample(), using 
  the same problem specification object. There is no need to call 
  \c GMRFLib_init_problem_hidden() more than once for each sampling problem. \n
  Prior to sampling, the built-in random number generator has to be initialized. 
  ...
  for details. \n
  At output, the sampling results are stored in the \c GMRFLib_hidden_problem_tp
  ...
  of \a hidden_problem after running the routine. The sampled values are stored in 
  <tt>hidden_problem->sample</tt> and the corresponding log-density in 
  <tt>hidden_problem->sub_logdens</tt>. The fixed values are copied into the 
...
int GMRFLib_sample_hidden(GMRFLib_hidden_problem_tp *hidden_problem)
{
    int retval;
    GMRFLib_ENTER_ROUTINE;
    retval = GMRFLib_doit_hidden(hidden_problem, 1);
    if (retval != GMRFLib_SUCCESS)
    {
	GMRFLib_LEAVE_ROUTINE;
	return retval;
    }
    retval = GMRFLib_doit_hidden(hidden_problem, 0);
    GMRFLib_LEAVE_ROUTINE;
    return retval;
}
...
/*!
  ...
  in the \em log_subdens member of the data structure \a hidden_problem.
...
int GMRFLib_doit_hidden(GMRFLib_hidden_problem_tp *hidden_problem, int sample_flag)
{
    ...
    */
    ...
    GMRFLib_uniform_setstate(h->ran_approx_state); 

    sub_n = h->sub_graph->n;
    if (!sub_n) return GMRFLib_SUCCESS;
    
    for(i=h->hidden_par->stop_idx;i<sub_n;i++) h->sub_sample[i] = h->sample[h->map[i]] - h->sub_mean[i];

    for(i=sub_n-1, h->sub_logdens=0.0;i>=h->hidden_par->stop_idx;i--) 
    {
	double ldens;
	GMRFLib_doit_hidden_i(h, sample_flag, i, &ldens);
	h->sub_logdens += ldens;
    }

    if (sample_flag) for(i=h->hidden_par->stop_idx;i<sub_n;i++) h->sample[h->map[i]] = h->sub_sample[i] + h->sub_mean[i];
    
    /* 
       switch back to the users state
    */
    GMRFLib_uniform_setstate(h->ran_current_state);
    FREE(h->ran_current_state);
...
int GMRFLib_doit_hidden_i(GMRFLib_hidden_problem_tp *h, int sample_flag, int idx, double *ldens)
{
    ...
    if (debug) printf("%s: enter with idx=%1d fidx=%1d\n", __GMRFLib_FuncName, idx, fidx);

    /* 
       setup workspace which is static & never free'ed
       FIXME: free this when calling ...free_hidden
       FIXME: dont need so large storage, only a subset will do.
    */
    if (h->hidden_par->nsample > num_sample || sub_n > len_sample || h->hidden_par->nantithetic > n_antithetic)
    {
	if (samples) for(i=0;i<num_sample;i++) FREE(samples[i]);
	if (sample_scale) for(i=0;i<num_sample;i++) FREE(sample_scale[i].s);
	FREE(samples); FREE(sample_scale); FREE(cmean0); FREE(cmean1); FREE(cmean); FREE(cmode);

	num_sample   = h->hidden_par->nsample;
	len_sample   = sub_n;
	n_antithetic = h->hidden_par->nantithetic;
	
	if (debug) printf("%s: malloc static storage for the samples\n", __GMRFLib_FuncName);

	if (num_sample)
	{
	    samples = Calloc(num_sample, double *); MEMCHK(samples);
	    for(i=0;i<num_sample;i++) 
	    {
		samples[i] = Calloc(len_sample, double); MEMCHK(samples[i]);
	    }
	}
	else
	{
	...
	cmean0 = Calloc(len_sample, double); MEMCHK(cmean0);
	cmean1 = Calloc(len_sample, double); MEMCHK(cmean1);
	cmean  = Calloc(len_sample, double); MEMCHK(cmean);
	cmode  = Calloc(len_sample, double); MEMCHK(cmode);

	if (num_sample)
	{
	    sample_scale = Calloc(num_sample, SampleScale_tp); MEMCHK(sample_scale);
	    for(i=0;i<num_sample;i++)
	    {
		...
		sample_scale[i].s = Calloc(sample_scale[i].n_sample_scale, double); MEMCHK(sample_scale[i].s);
	    }
	}
	else
	{
    ...
    /* 
       first we need to sample the Gaussians with zero conditional mean. cmean is the conditonal
       mean for a fixed value of x[idx], any choice will do. then we subtract cond.mean from the
       samples to obtain zero cond.mean ones
    */
    for(i=idx+1;i<MIN(idx+bw+1, sub_n);i++) cmean0[i] = cmean1[i] = cmode[i] = h->sub_sample[i];
    for(i=idx-1;i>=fidx;i--) cmean0[i] = cmean1[i] = cmode[i] = 0.0;
    cmean0[idx] = 0.0;				  /* fixed value */
    cmean1[idx] = 1.0;				  /* fixed value */
    GMRFLib_solve_lt_sparse_matrix_special(cmean0, &(h->sub_sm_fact), h->sub_graph, idx, fidx, 1); /* no remap as remap=I */
    GMRFLib_solve_lt_sparse_matrix_special(cmean1, &(h->sub_sm_fact), h->sub_graph, idx, fidx, 1);
    ...
    */
	...
	if (1 ||					  /* do this always, se below why */
	    h->hidden_par->neightype == GMRFLib_NEIGHTYPE_LINEAR ||
	    h->hidden_par->cmeanmode == GMRFLib_COND_MODE) /* FIXME: is this what I whant? */
	{
	    /* 
	       sample every index
	    */
		...
		for(j=idx+1;j<MIN(idx+bw+1, sub_n);j++) samples[i][j] = cmean0[j]; 
		...
		if (h->hidden_par->nsample == 1 && h->hidden_par->nantithetic == 0)
		{
		    ...
		    */
		    for(j=idx-1;j>=fidx;j--) samples[i][j] = 0.0;
		}
		else
		{
		    /* 
		    ...
		    */
		    ...
		    {
			fac = 1.0/sqrt(nsqr);
			for(j=idx-1, nsqr=0.0;j>=fidx;j--) samples[i][j] *= fac;
		    }
		}
		...
		if (idx -fidx > 0)
		{
			...
			for(jj=0, kk=0;jj< h->hidden_par->nantithetic;jj++)
			{
			    ...
			    if (v > 1.0) v -= 1.0;
			    sample_scale[i].s[kk]   =  sqrt(GMRFLib_invChiSquare(v,(double)(idx-fidx))); 
			    sample_scale[i].s[kk+1] = -sample_scale[i].s[kk];
			    sample_scale[i].s[kk+2] =  sqrt(GMRFLib_invChiSquare(1.0-v,(double)(idx-fidx))); 
			...
			sample_scale[i].s[0] = sqrt(GMRFLib_invChiSquare(GMRFLib_uniform(),(double)(idx-fidx))); 
		}
		else
		    memset(sample_scale[i].s, 0, sample_scale[i].n_sample_scale*sizeof(double));
		
		GMRFLib_solve_lt_sparse_matrix_special(samples[i], &(h->sub_sm_fact), h->sub_graph, idx, fidx, 1);	
		for(j=idx;j>=fidx;j--) samples[i][j] -= cmean0[j];
	    }
	}
	else					  /* == GMRFLib_NEIGHTYPE_GRAPH */
	{
	    ...
	    */
	    ...
	    /* 
	       dette er ikke helt rett ettersom vi maa betinge mhp paa hele x i bandwidth, og ikke
	       bare x[idx]!!! men vi kan finne L (hvor Lz=x) ved aa se paa sampler med z=0 og z_i=1
	       deretter. da finner vi vel kolonne 'i' i L eller noe.

	       profile'ing viser at ikke saa mye effort gaar med paa dtbsvspecial_, saa er det
	       egentlig saa mye vits? dette er et realistisk eksempel:

	       Each sample counts as 0.01 seconds.
	       %   cumulative   self              self     total           
	       time   seconds   seconds    calls  ms/call  ms/call  name    
	       33.35    56.49    56.49 505138953     0.00     0.00  GMRFLib_loglFunc_wrapper
	       29.30   106.12    49.63    187392     0.26     0.81  GMRFLib_ConditionalFunc
	       27.03   151.90    45.78 505245459     0.00     0.00  loglik
	       3.22    157.36     5.46   2962186     0.00     0.00  gratio_
	       1.91    160.60     3.24     40992     0.08     0.08  dtbsvspecial_
	    ...
	    */
		...
		if (h->hidden_par->ngraph->nbs[idx][j] < idx) nneig++;
	    ndim  = nneig+1;
	    cndim = nneig;

	    if (cndim > 0)
	    {
		sol   = Calloc(sub_n, double); MEMCHK(sol);
		cov   = Calloc(SQR(ndim), double); MEMCHK(cov);
		ccov  = Calloc(SQR(cndim), double); MEMCHK(ccov);
		indexs = Calloc(ndim, int); MEMCHK(indexs);
		...
		for(j=0;j<ndim;j++)
		{
		    ...
		    sol[indexs[j]] = 1.0;
		    GMRFLib_solve_llt_sparse_matrix(sol, &(h->sub_sm_fact), h->sub_graph);
		    for(i=0;i<ndim;i++) cov[j + i*ndim] = cov[i + j*ndim] = sol[indexs[i]];
		}
		for(j=0;j<cndim;j++)		  
		    for(i=0;i<cndim;i++)
		...
		GMRFLib_comp_chol_general(&chol_ccov, ccov, cndim, NULL, GMRFLib_ESINGMAT);

		for(i=0;i<h->hidden_par->nsample;i++)
		{
		    for(j=0;j<cndim;j++) sol[j] = GMRFLib_stdnormal();
		    for(ii=0;ii<cndim;ii++)
		    {
			for(jj=0, hold=0.0;jj<=ii;jj++) hold += chol_ccov[ii+jj*cndim]*sol[jj];
			samples[i][indexs[ii+1]] = hold;
		    }
		}

		FREE(sol); FREE(cov); FREE(ccov); FREE(indexs); FREE(chol_ccov);
	    }
	}
    }
    
    if (!args) args = Calloc(1, GMRFLib_ConditionalFunc_arg_tp);
    ...
    {
	GMRFLib_comp_cond_meansd(&cm, &csd, idx, cmode, 1, &(h->sub_sm_fact), h->sub_graph);
    }
    else
    {
	if (h->hidden_par->cmeanmode == GMRFLib_COND_MODE)
	    GMRFLib_locate_cmode(cmode, idx, h, &cm, &csd);
	else
	{	
	    GMRFLib_comp_cond_meansd(&cm, &csd, idx, cmode, 1, &(h->sub_sm_fact), h->sub_graph);
	    cmode[idx] = cm;
	...
	if (1)
	{
	    /* 
	       old
	    */
	    gdens = GDensInit(x_low, x_high, h->hidden_par->nresolution, h->hidden_par->nresolution,
			      h->hidden_par->norder, GMRFLib_ConditionalFunc, (char *)args);
	}
	else
	{
	    /* 
	    ...
	    */
	    gdens = GDensInitNew(h->hidden_par->range, cmode[idx], csd, h->hidden_par->nresolution,
				 GMRFLib_ConditionalFunc, (char *)args);
	}
	...
	/* 
	   ...oops, here we need to be a bit carful about the random stream. if we want to sample,
	   we doit from the 'current' state, ie we need to switch to that one just here, and then
	   return to the 'approx_state' afterwards.
	*/
	...
	{
	    GDensSample(gdens, &(h->sub_sample[idx]), ldens);
	}
	...
	*/
	...
	{
	    GDensLDens(gdens, &(h->sub_sample[idx]), ldens);
	}
    }
	...
	*(h->hidden_par->marginal_density) = gdens;
	if (h->hidden_par->marginal_mean)  *(h->hidden_par->marginal_mean)  = h->sub_mean[idx];
	if (h->hidden_par->marginal_stdev) *(h->hidden_par->marginal_stdev) = csd;
    }
    else
	GDensFree(gdens);
...
int GMRFLib_locate_cmode(double *cmode, int idx, GMRFLib_hidden_problem_tp *h,
			 double *cond_mean, double *cond_stdev)
{
    /* 
       locate the conditonal mode for 'idx' taking the data also into account.  if cond_mean then
       return the conditonal mean and similar with cond_stdev
    */
    double xopt, hivar, cm, csd, fac;
    int iter_max=50;
 
    GMRFLib_comp_cond_meansd(&cm, &csd, idx, cmode, 1, &(h->sub_sm_fact), h->sub_graph);
    if (cond_mean)  *cond_mean  = cm;
    if (cond_stdev) *cond_stdev = csd;
	...
	if (!ISZERO(dderiv))
	{
	    xopt -= MIN(fac, 1.0)*deriv/dderiv;		  /* ...because 1/step's cancels here */
	    if (ABS(deriv/dderiv) < eps) break;
	}
	fac += 0.25;
    }
	...
	cmode[idx] = cm;
    if (0) GMRFLib_ASSERT(iter_max > 0, GMRFLib_EOPTNR);


    return GMRFLib_SUCCESS;
}
double GMRFLib_ConditionalFunc(double xn, char *arg)
{
	...
	cond_stdev = Calloc(len_cond_stdev, double); MEMCHK(cond_stdev);
    }
	...
	FREE(ldens_f);
	ldens_f = Calloc(len_ldens_f, double); MEMCHK(ldens_f);
    }
	...
	ff = Calloc(len_xx, double); MEMCHK(ff);
	xx = Calloc(len_xx, double); MEMCHK(xx);
    }
    ...
    */
    ...
    {
	a->cmode[a->idx] = xn;
	for(j=a->idx-1;j>=a->fidx;j--) GMRFLib_locate_cmode(a->cmode, j, a->h, NULL, &cond_stdev[j]);
    ...
    if ((a->h->hidden_par->neightype == GMRFLib_NEIGHTYPE_LINEAR) ||
	(a->h->hidden_par->cmeanmode == GMRFLib_COND_MODE))
    {						  /* need all of them */
	for(j=a->idx-1;j>=a->fidx;j--) a->cmean[j] = a->cmean0[j] + fac*(a->cmean1[j] - a->cmean0[j]);
    }
    else					  /* do not need all cmean's */
    {
	if (0) for(j=a->idx+1;j>=a->fidx;j--) a->cmean[j] = DBL_MAX; /* just a test... */
	...
	if (a->h->hidden_par->cmeanmode == GMRFLib_COND_MODE) /* only needed then */
	{
	    ...
	    if (0)
		for(k=0;k<a->h->hidden_par->nsample;k++)
		    for(jj=0;jj<ns;jj++) printf("correction for cmode/mean %d %f\n", kk+jj, ldens_f[kk+jj]);
	}
	for(j=0; j<a->h->hidden_par->ngraph->nnbs[a->idx];j++)
	{
		...
		*/
		...
		for(k=0, kk=0;k<a->h->hidden_par->nsample;k++)
			...
			if (0) printf("ff[%1d] %f ds %f\n", kk, ldens_f[kk], d_s);
			kk++;
		    }
	    }
	}
	
	for(i=0, sum=0.0;i<a->h->hidden_par->nsample*ns;i++) sum += exp(ldens_f[i]);
    ...
    */
    ...
    if (0) printf("xn %f cm %f csd %f ldens %f \n", xn, cm, cond_stdev[a->idx], ldens);
