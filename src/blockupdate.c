...
static const char RCSId[] = "$Id: blockupdate.c 1 2013-03-28 13:54:24Z hanne $";
...
/*!
  ...
  If \c #GMRFLib_MODEOPTION_MODE, the mode of the posterior distribution is 
  estimated and the expansion is done around the mode. \n
  If \c #GMRFLib_MODEOPTION_CURRENT, the expansion is done around the current 
  value of <em>\b x</em>. \n
  0 = mode, 1 = current \n
  <b>Default value: 0 </b> \n\n
...
int GMRFLib_default_blockupdate_param(GMRFLib_blockupdate_param_tp **blockupdate_par)
{
    ...
    *blockupdate_par                  = Calloc(1, GMRFLib_blockupdate_param_tp); MEMCHK(*blockupdate_par);
    (*blockupdate_par)->modeoption    = GMRFLib_MODEOPTION_MODE;
    (*blockupdate_par)->fp            = NULL;
    (*blockupdate_par)->step_len      = 1.0e-4;
...
/*!
  ...
  \brief Generate a blockproposal using a GMRF-approximation for a GMRF conditioned on (possible)
  non-Gaussian observations.

  \param[out] laccept The log of the acceptance rate of the proposed (new) value of <em>\b x</em>,
  returned by \a x_new.  \param[out] x_new The proposed values of <em>\b x</em>, denoted by <em>\b
  x'</em>.

  \param[in] x_old The current (old) update of <em>\b x</em>.

  \param[in] b_new The value of <em>\b b</em> in <b>(GMRF-30)</b> in \ref block, based on proposed
  values of the hyper-parameters, \f$ \mbox{\boldmath $\theta$}^{'} \f$.  If \c NULL, <em>\b b</em>
  is assumed to be 0.

  \param[in] b_old The value of <em>\b b</em> in <b>(GMRF-30)</b> in \ref block,  based on current (old) values of
  the hyper-parameters, \f$ \mbox{\boldmath $\theta$} \f$.  If \c NULL, <em>\b b</em> is assumed to
  be 0.

  \param[in] c_new The value of <em>\b c</em> in <b>(GMRF-30)</b> in \ref block, based on proposed
  values of the hyper-parameters, \f$ \mbox{\boldmath $\theta$}^{'} \f$.  If \c NULL, <em>\b c</em>
  is assumed to be 0.

  \param[in] c_old The value of <em>\b c</em> in <b>(GMRF-30)</b>  in based on current (old) values of
  the hyper-parameters, \f$ \mbox{\boldmath $\theta$} \f$. If \c NULL, <em>\b c</em> is assumed to
  be 0.

  \param[in] mean_new The value of \f$ \mbox{\boldmath $\mu$} \f$ in <b>(GMRF-30)</b> in \ref block,
  based on proposed values of the hyper-parameters, \f$ \mbox{\boldmath $\theta$}^{'} \f$. If \c
  NULL, \f$ \mbox{\boldmath $\mu$} \f$ is assumed to be 0.

  \param[in] mean_old The value of \f$ \mbox{\boldmath $\mu$} \f$ in <b>(GMRF-30)</b> in \ref block,
  based on current (old) values of the hyper-parameters, \f$ \mbox{\boldmath $\theta$} \f$. If \c
  NULL, \f$ \mbox{\boldmath $\mu$} \f$ is assumed to be 0.

  \param[in] d_new The value of <em>\b d</em> in <b>(GMRF-30)</b> in \ref block, based on proposed
  values of the hyper-parameters, \f$ \mbox{\boldmath $\theta$}^{'} \f$. If \c NULL, <em>\b d</em>
  is assumed to be 0 and no optimizing is done (or needed).

  \param[in] d_old The value of <em>\b d</em> in <b>(GMRF-30)</b> in \ref block, based on current (old)
  values of the hyper-parameters, \f$ \mbox{\boldmath $\theta$} \f$. If \c NULL, <em>\b d</em> is
  assumed to be 0 and no optimizing is done (or needed).

  \param[in] loglFunc_new A function of type \c GMRFLib_logl_tp(), returning the value of the
  function \f$ f(x_i) \f$ in <b>(GMRF-30)</b> in \ref block, in many applications equal to the
  log-likelihood of the problem. If the function \f$ f(x_i) \f$ depends on unknown hyper-parameters,
  this function uses the proposed values \f$ \mbox{\boldmath $\theta$}^{'} \f$.

  \param[in] loglFunc_arg_new A \em character pointer holding the address of a variable or data
  structure defining additional arguments to the function \a loglFunc_new.

  \param[in] loglFunc_old A function of type \c GMRFLib_logl_tp(), returning the value of the
  function \f$ f(x_i) \f$ in <b>(GMRF-30)</b> in \ref block, in many applications equal to the log-likelihood of
  the problem. If the function \f$ f(x_i) \f$ depends on unknown hyper-parameters, this function
  uses the current values \f$ \mbox{\boldmath $\theta$} \f$, if not, this function should be equal
  to \a loglFunc_new.

  \param[in] loglFunc_arg_old A \em character pointer holding the address of a variable or data
  structure defining additional arguments to the function \a loglFunc_old.

  \param[in] fixed_value If \c !NULL, the block-sampling of <em>\b x</em> is done conditionally on
  fixed values.  The elements of this array, of length \c graph->n, should take the value 0 or
  1. The conditional mean and covariance matrix of \f$ \{x_i:\mbox{\small\tt fixed\_value}[i]=0\}
  ...
  values specified in the argument \a x_old. It is allowed that all values of <tt>fixed_value</tt>
  equals 1.

  \param[in] graph The graph on which the GMRF <em>\b x</em> is defined.

  \param[in] Qfunc_new A function of type \c GMRFLib_Qfunc_tp(), computing the values of the
  precision matrix <em>\b Q</em>. If the function depends on unknown hyper-parameters, this function
  computes the precision matrix as a function of their proposed values \f$ \mbox{\boldmath
  $\theta$}^{'} \f$.

  \param[in] Qfunc_arg_new A \em character pointer holding the address of a variable or data
  structure defining additional arguments to the function \a Qfunc_new.

  \param[in] Qfunc_old A function of type \c GMRFLib_Qfunc_tp(), computing the values of the
  precision matrix <em>\b Q</em>. If the function depends on unknown hyper-parameters, this function
  computes the precision matrix as a function of their current (old) values \f$ \mbox{\boldmath
  $\theta$} \f$. If not, this function should be equal to \a Qfunc_new.

  \param[in] Qfunc_arg_old A \em character pointer holding the address of a variable or data
  structure defining additional arguments to the function \a Qfunc_old.

  \param[in] Qfunc_old2new This function implements the <em>\b Q</em>-matrix used when proposing a
  new value <em>\b x'</em> given the current value <em>\b x</em>. It is needed only in cases where
  the precision matrix depends on unknown hyper-parameters, and is different from the matrix \a
  Qfunc_new. If \c NULL, \a Qfunc_old2new is set equal to \a Qfunc_new.

  \param[in] Qfunc_arg_old2new A \em character pointer holding the address of a variable or data
  structure defining additional arguments to the function \a Qfunc_old2new.

  \param[in] Qfunc_new2old This function implements the <em>\b Q</em>-matrix for the reverse of the
  proposal process, going from the proposed value <em>\b x'</em> to the the current value <em>\b
  x</em>. It is needed only in cases where the precision matrix depends on unknown hyper-parameters,
  and is different from the matrix \a Qfunc_old. If \c NULL, \a Qfunc_new2old is set equal to \a
  Qfunc_old.

  \param[in] Qfunc_arg_new2old A \em character pointer holding the address of a variable or data
  structure defining additional arguments to the function \a Qfunc_new2old.

  \param[in] constr_new A pointer to a \c GMRFLib_constr_tp -object holding the information on the
  type of linear constraint, in the case of constrained sampling. If the parameters of the
  constraint depend on unknown hyper-parameters, \a constr_new should define the constraint
  corresponding to the proposed values \f$ \mbox{\boldmath $\theta$}^{'} \f$.

  \param[in] constr_old A pointer to a \c GMRFLib_constr_tp -object holding the information on the
  type of linear constraint, in the case of constrained sampling. If the parameters of the
  constraint depend on unknown hyper-parameters, \a constr_old should define the constraint
  corresponding to the current values \f$ \mbox{\boldmath $\theta$} \f$. If the constraint does not
  depend on the hyper-parameters, \a constr_old and \a constr_new should be equal.
  ...
  \remarks All parameters in <b>(GMRF-30)</b> in \ref block might depend on unknown hyper-parameters
  \f$ \mbox{\boldmath $\theta$} \f$, such that \f$ \mbox{\boldmath $b=b(\theta)$} \f$ and so
  on. These hyper-parameters are also to be updated by the MCMC algorithm, and the contributions to
  the acceptance rate from these parameters are to be added to \a laccept after invoking
  GMRFLib_blockupdate(). If the parameters in <b>(GMRF-30)</b> in \ref block, depend on \f$
  \mbox{\boldmath $\theta$} \f$, two sets of parameters have to be provided, one depending on the
  current (old) value of \f$ \mbox{\boldmath $\theta$} \f$, and one depending on the proposed values
  \f$ \mbox{\boldmath $\theta$}^{'} \f$, that are assumed to be generated prior to calling
  GMRFLib_blockupdate(). The two sets of values take subscripts \c new (corresponding to the
  proposals \f$ \mbox{\boldmath $\theta$}^{'} \f$) and \c old (corresponding to the current, or old,
  values \f$ \mbox{\boldmath $\theta$} \f$). The same comment applies to the set of function
  defining the precision matrix <em>\b Q</em>, these functions might also depend on
  hyper-parameters.\n\n For the \em Q-functions, two additional functions are provided. These are
  needed for generating the proposals from the Gaussian proposal distribution in cases where the
  precision matrix depend on unknown hyper-parameters, and the precision matrix of the proposal
  distribution is different from the precision matrix <em>\b Q</em> of the GMRF at input. This might
  be the case when \f$ \mbox{\boldmath $Q(\theta)$} + \mbox{diag}(\mbox{\boldmath $c$}) \f$ is
  singular. The function \a GMRFLib_Qfunc_old2new is the <em>\b Q</em>-matrix for the distribution
  generating a proposal <em>\b x'</em> given <em>\b x</em>, and \a GMRFLib_Qfunc_new2old is the
  corresponding function, implementing the <em>\b Q</em>-matrix used in the reverse process, going
  from <em>\b x'</em> to <em>\b x</em>.
			...
			GMRFLib_blockupdate_param_tp *blockupdate_par)
{
    ...
    retval = GMRFLib_blockupdate_store(laccept, x_new, x_old, b_new, b_old, c_new, c_old, mean_new, mean_old, d_new, d_old,
					loglFunc_new, loglFunc_arg_new, loglFunc_old, loglFunc_arg_old, fixed_value, graph,
					Qfunc_new, Qfunc_arg_new, Qfunc_old, Qfunc_arg_old, Qfunc_old2new, Qfunc_arg_old2new,
					Qfunc_new2old, Qfunc_arg_new2old, constr_new, constr_old, optpar, blockupdate_par, NULL);
    GMRFLib_LEAVE_ROUTINE;
    return retval;
}
/*!
  \brief Generate a blockproposal using a GMRF-approximation for a GMRF conditioned on (possible)
  non-Gaussian observations storing and reusing computations for increased speed.


   This function is the same as \c GMRFLib_blockupdate() except an additional argument which
  holds computations that can be resused.

  \param[in,out] store A pointer to a \c GMRFLib_store_tp object for which temporary calculations will
  be stored and retrieved to increase the speed compared to the \c GMRFLib_blockupdate() routine.
  
  \sa GMRFLib_blockupdate, GMRFLib_free_store
  
  This function provide a tool for reusing calculations when \c GMRFLib_blockupdate() is used
  repeatedly on the same problem, over and over again. The gain is a significant increase in speed.

  Calculations can be reused at two levels, depending on the setting which this is used.

  - The first level reuse the reordering, the sub_graph and the symbolic factorization of the
    precision matrix. This is the default option.
  - The second level, also reuse \c GMRFLib_problem_tp objects computed. This is possible for some,
    but not all, cases where \c GMRFLib_blockupdate_store() can be used. This level must be
    activated by the user by setting \c store->store_problems to 1 or \c
    #GMRFLib_TRUE. Further, information if the proposal was accepted or rejected must be provided,
    setting \c store->decision to \c #GMRFLib_STORE_ACCEPT or \c #GMRFLib_STORE_REJECT.
...
{
    /* 
       do a blockupdate, and return the proposed new state in 'x' and the corresponding
       log-acceptrate in 'laccept'

       the density is

       exp[ -1/2 (x-mean)'(Q+diag(c))(x-mean) + b'x + \sum d_i f(x_i) ]

       where values are fixed if fixed_value[i] are true, and where a constraint, Ax=b can be
       spesified in 'constr' with optional noise

       it's now ok that all values in fixed are 1, then none are updated, just the acceptrate is
       computed. 

    */

    int n, i, free_block;
    double *mode=NULL, *bb=NULL, *cc=NULL, old2new, new2old, old, new, *xx, *yy, logll;
    GMRFLib_problem_tp *problem = NULL;
    GMRFLib_blockupdate_param_tp *blockpar;
    ...
    if (constr_new) GMRFLib_prepare_constr(constr_new, graph, 0); /* no scaleing */
    if (constr_old) GMRFLib_prepare_constr(constr_old, graph, 0); /* no scaleing */
    ...
    */
	...
	if (!Qfunc_arg_old2new) Qfunc_arg_old2new = Qfunc_arg_new;
    }
    if (!Qfunc_new2old)
    {
	Qfunc_new2old = Qfunc_old;		  
	if (!Qfunc_arg_new2old) Qfunc_arg_new2old = Qfunc_arg_old;
    }
    ...
    n    = graph->n;
    mode = Calloc(n, double); MEMCHK(mode);
    xx   = bb = Calloc(n, double); MEMCHK(bb);	  /* two names for the same storage */
    yy   = cc = Calloc(n, double); MEMCHK(cc);	  /* two names for the same storage */
    ...
    */
	...
	{
	    if (store->problem_new2old) GMRFLib_free_problem(store->problem_new2old);
	    store->problem_new2old = store->problem_old2new;
	    store->problem_old2new = NULL;

	    if (store->old && store->new)
		*(store->old) = *(store->new);	  /* swap the log-density */
	}
	if (store->decision == GMRFLib_STORE_REJECT)
	{
	    if (store->problem_old2new) GMRFLib_free_problem(store->problem_old2new);
	    store->problem_old2new = NULL;
	}
    }
    ...
    */
	...
	GMRFLib_optimize_store(mode, b_new, c_new, mean_new, graph, 
    ...
    */
	...
	for(i=0;i<n;i++)
		...
		cc[i] = MAX(0.0, cc[i]);	  /* do not want negative terms on the diagonal */
	    }
    
    
    /* 
       add the linear and quadratic term to the general model. note that we need to correct the
       b-term due to the mean.
    */
    if (b_new)
    {
	if (mean_new) 
	    for(i=0;i<n;i++) bb[i] += b_new[i] - cc[i]*mean_new[i];
	else
	    for(i=0;i<n;i++) bb[i] += b_new[i];
    }
    else
    {
	if (mean_new) 
	    for(i=0;i<n;i++) bb[i] -= cc[i]*mean_new[i];
    }
	
    if (c_new) for(i=0;i<n;i++) cc[i] += c_new[i];

    GMRFLib_init_problem_store(&problem, x_old, bb, cc, mean_new, graph,
	...
	*/
	...
	if (blockpar->modeoption == GMRFLib_MODEOPTION_MODE && d_old)
	    GMRFLib_optimize_store(mode, b_old, c_old, mean_old, graph, 
	...
	*/
	...
	{
	    if (mean_old) 
		for(i=0;i<n;i++) bb[i] += b_old[i] - cc[i]*mean_old[i];
	    else
		for(i=0;i<n;i++) bb[i] += b_old[i];
	}
	else
	{
	    if (mean_old) 
		for(i=0;i<n;i++) bb[i] -= cc[i]*mean_old[i];
	}
	if (c_old) for(i=0;i<n;i++) cc[i] += c_old[i];
    
	GMRFLib_init_problem_store(&problem, x_new, bb, cc, mean_old, graph, Qfunc_new2old, Qfunc_arg_new2old,
				   fixed_value, constr_old, GMRFLib_NEW_PROBLEM, store);
    }
	...
	*/
    ...
    /* 
       ...
       FIXME: here i do not use subgraph, but that require big tests to see if the results would be
       the same. is it worth it really?
    */

    new=0.0;
    if (mean_new)
	for(i=0;i<n;i++) xx[i] = x_new[i] - mean_new[i];
    else
	memcpy(xx, x_new, n*sizeof(double));
    GMRFLib_Qx(yy, xx, graph, Qfunc_new, Qfunc_arg_new);
    if (c_new)
	for(i=0;i<n;i++) new += yy[i]*xx[i] + c_new[i]*SQR(xx[i]);
    else
	for(i=0;i<n;i++) new += yy[i]*xx[i];
    new *= -0.5;
    if (b_new) for(i=0;i<n;i++) new += b_new[i]*x_new[i];
    if (d_new)
	for(i=0;i<n;i++) 
	    if (d_new[i]) 
	    {
    ...
    {
	double sqr_term;
	GMRFLib_eval_constr(NULL, &sqr_term, x_new, constr_new, graph);
	new += -0.5*sqr_term;
    }
    ...
    */
	...
	if (!(store->new))
	{
	    store->new = Calloc(1, double); MEMCHK(store->new);
	}
	*(store->new) = new;
    }

    if (store && store->store_problems && store->old)
    {
	old = *(store->old);
    }
    else
	...
	old=0.0;
	if (mean_old)
	    for(i=0;i<n;i++) xx[i] = x_old[i] - mean_old[i];
	else
	    memcpy(xx, x_old, n*sizeof(double));
	GMRFLib_Qx(yy, xx, graph, Qfunc_old, Qfunc_arg_old);
	if (c_old)
	    for(i=0;i<n;i++) old += yy[i]*xx[i] + c_old[i]*SQR(xx[i]);
	else
	    for(i=0;i<n;i++) old += yy[i]*xx[i];
	old *= -0.5;
	if (b_old) for(i=0;i<n;i++) old += b_old[i]*x_old[i];
	if (d_old)
	    for(i=0;i<n;i++)
		if (d_old[i]) 
		{
	...
	{
	    double sqr_term;
	    GMRFLib_eval_constr(NULL, &sqr_term, x_old, constr_old, graph);
	    old += -0.5*sqr_term;
	}
    }
    ...
    */
	...
	if (!(store->old))
	{
	    store->old = Calloc(1, double); MEMCHK(store->old);
	}
	*(store->old) = old;
    }
    ...
    */
    *laccept = new + new2old - (old + old2new);
    ...
    FREE(xx); FREE(yy); FREE(mode); if (free_block) FREE(blockpar);
...
/*!
  ...
  \param[in] loglFunc_arg A \em character pointer holding the address of a variable or data
  ...
  \param[in] Qfunc_arg A \em character pointer holding the address of a variable or data structure
				    ...
				    GMRFLib_constr_tp *constr, GMRFLib_optimize_param_tp *optpar, GMRFLib_blockupdate_param_tp *blockupdate_par)
{
    int retval;
    GMRFLib_ENTER_ROUTINE;
    retval = GMRFLib_init_GMRF_approximation_store(problem, x, b, c, mean, d, loglFunc, loglFunc_arg, fixed_value,
...
int GMRFLib_init_GMRF_approximation_store(GMRFLib_problem_tp **problem, double *x, double *b, double *c, double *mean, double *d, 
					   GMRFLib_logl_tp *loglFunc, char *loglFunc_arg, char *fixed_value,
					   GMRFLib_graph_tp *graph, GMRFLib_Qfunc_tp *Qfunc, char *Qfunc_arg,    
					   GMRFLib_constr_tp *constr, GMRFLib_optimize_param_tp *optpar, GMRFLib_blockupdate_param_tp *blockupdate_par,
					   GMRFLib_store_tp *store)
{
    int retval, i, free_x=0, free_b=0, free_c=0, free_mean=0, free_d=0, free_blockpar=0, n;
    double *bb, *cc, *mode;
...
#define FREE_ALL if (1) { if (free_x) FREE(x); if (free_b) FREE(b); if (free_c) FREE(c); if (free_d) FREE(d); \
    ...
    GMRFLib_ASSERT(problem, GMRFLib_EINVARG); GMRFLib_ASSERT(graph, GMRFLib_EINVARG); GMRFLib_ASSERT(Qfunc, GMRFLib_EINVARG);
    GMRFLib_ENTER_ROUTINE;

    FIXME("GMRFLib_init_GMRF_approximation_store is not properly tested");
	...
	*problem = NULL;
    ...
    if (!x)    { x    = Calloc(n, double); MEMCHK(x);    free_x    = 1;} 
    if (!b)    { b    = Calloc(n, double); MEMCHK(b);    free_b    = 1;} 
    if (!c)    { c    = Calloc(n, double); MEMCHK(c);    free_c    = 1;} 
    if (!d)    { d    = Calloc(n, double); MEMCHK(d);    free_d    = 1;} 
    if (!mean) { mean = Calloc(n, double); MEMCHK(mean); free_mean = 1;} 
    
    bb   = Calloc(n, double); MEMCHK(bb);
    cc   = Calloc(n, double); MEMCHK(cc);
    mode = Calloc(n, double); MEMCHK(mode);
    ...
    {
	retval = GMRFLib_optimize_store(mode, b, c, mean, graph, Qfunc, Qfunc_arg, fixed_value, constr, d, loglFunc, loglFunc_arg, optpar, store);
    ...
    */
	...
	for(i=0;i<n;i++)
	    ...
	    {
		GMRFLib_2order_approx(NULL, &bb[i], &cc[i], d[i], mode[i], i,
				      mode, loglFunc, loglFunc_arg, &(blockupdate_par->step_len));
		cc[i] = MAX(0.0, cc[i]);	  /* do not want negative terms on the diagonal */
	    }
    ...
    {
	if (mean) 
	    for(i=0;i<n;i++) bb[i] += b[i] - cc[i]*mean[i];
	else
    ...
    {
	if (mean) 
	    for(i=0;i<n;i++) bb[i] -= cc[i]*mean[i];
    }
    if (c) for(i=0;i<n;i++) cc[i] += c[i];
    
    retval = GMRFLib_init_problem_store(problem, x, bb, cc, mean, graph, Qfunc, Qfunc_arg, fixed_value, constr, GMRFLib_NEW_PROBLEM, store);
			  ...
			  double *step_len)
{
    ...
    step = (step_len && *step_len > 0.0 ? *step_len : 1.0e-4);
			  ...
			  double *step_len)
{
    ...
    if (a) *a = d*(f[1] -df*x0 +0.5*ddf*SQR(x0));
    if (b) *b = d*(df - x0*ddf);
    if (c) *c = -d*ddf;
			       ...
			       GMRFLib_hidden_param_tp *hidden_par)
{
    int retval;
    GMRFLib_ENTER_ROUTINE;
    retval = GMRFLib_blockupdate_hidden_store(laccept, x_new, x_old, b_new, b_old, c_new, c_old, mean_new, mean_old,
					       d_new, d_old, loglFunc_new, loglFunc_arg_new, loglFunc_old, loglFunc_arg_old,
					       fixed_value, graph, Qfunc_new, Qfunc_arg_new, Qfunc_old, Qfunc_arg_old,
					       Qfunc_old2new, Qfunc_arg_old2new, Qfunc_new2old, Qfunc_arg_new2old,
					       optpar, hidden_par, NULL);
    GMRFLib_LEAVE_ROUTINE;
    return retval;
}
				      ...
				      GMRFLib_store_tp *store)
{
    ...
    */
    ...
    if (!Qfunc_old2new)     Qfunc_old2new     = Qfunc_new;
    if (!Qfunc_arg_old2new) Qfunc_arg_old2new = Qfunc_arg_new;
    if (!Qfunc_new2old)     Qfunc_new2old     = Qfunc_old;
    if (!Qfunc_arg_new2old) Qfunc_arg_new2old = Qfunc_arg_old;
    ...
    GMRFLib_init_problem_hidden_store(&hidden_problem,
    ...
    GMRFLib_sample_hidden(hidden_problem);
    ...
    GMRFLib_evaluate_hidden(hidden_problem);
...
library. Note how the \em char* argument to various functions is used to transfer arguments.
