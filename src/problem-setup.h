 ...
 *
 * RCSId: $Id: problem-setup.h 1 2013-03-28 13:54:24Z hanne $
 *
 */
/*!
...
/*! 
  \struct GMRFLib_constr_intern_tp problem-setup.h
...
{						  
    ...
    */
    ...
    /*!
      ...
      A length <tt>(nc * n)</tt> array holding the elements of the <tt>(nc * n)</tt> matrix <em>\b
      A</em> in the linear constraint <em>\b Ax = \b e</em>. The matrix is assumed to be stored
      column by column, that is, element <tt>i + nc * j</tt> of \a a_matrix should correspond to
      <em>\b A</em>[i,j]. \n\n
    */
    double *a_matrix;				  
    ...
    /*!
      ...
      A length \c nc array holding the elements of the vector <em>\b e</em> of the constraint <em>\b
      Ax = \b e</em>. \n\n
    */
    double *e_vector;				  
    ...
    /*!
      ...
      If <tt>!=NULL</tt>, the covariance matrix \f$ \mbox{\boldmath $\Sigma$}_{\epsilon} \f$ of the
      error \f$ \mbox{\boldmath $\epsilon$} \f$ is assumed to be diagonal, and \a errcov_diagonal is
      a length \c nc -array containing the diagonal elements.\n\n
    */
    ...
    /*!
      ...
      If <tt>!=NULL</tt>, \a errcov_general specifies a general positive definite covariance matrix
      \f$ \mbox{\boldmath $\Sigma$}_{\epsilon} \f$ of the error \f$ \mbox{\boldmath $\epsilon$}
      \f$. If both \a errcov_general and \a errcov_diagonal are <tt>!NULL</tt>, the matrix is
      assumed to be diagonal, and \a errcov_diagonal is used. \n\n
    */
    ...
    */
    GMRFLib_constr_intern_tp *intern;		  
}
GMRFLib_constr_tp;

typedef struct
{
    char *user_Qfunc_args;
...
typedef struct
{
    ...
    */
    ...
    /*!
      \brief It symbolic factorisation
    */
...
typedef struct
{
    /*!
      ...
      The inverse of \a Q, \$S=(S_{ij})\$, are computed for those \a i and \a j where \$L_{ij}\$ is non-zero.
      The values of \$S_{ij}\$ are store in internal (reordered) coordinates.
...
/*! 
  ...
  are needed by the user for extracting the results of the sampling routines. The members \a
  sub_mean and \a sub_mean_constr are of length \a n_sub, where \a n_sub is the number of elements
  of the GMRF <em>\b x</em> that are updated during the sampling. Which elements of <em>\b x</em>
  that are updated, is given by the index vector \a map. More specifically, the elements <em>
  map[i]; i=0,..., n_sub-1</em> of <em>\b x</em> are updated, and these correspond to the elements
  <em>i=0,..., n_sub-1</em> of the mean vectors \a sub_mean and \a sub_mean_constr.
...
typedef struct
{
    /*!
      ...
      A length \em n array containing a sample of the GMRF. If some of the elements of <em>\b x</em>
      are fixed, these fixed values are copied into the corresponding positions in \c sample, while
      the rest are samples (or configuration to be evalutated) from the conditional distribution
      given the fixed values.

      This array is just \ref GMRFLib_problem_tp::sub_sample copied into the correct locations,
      where the rest is fixed values.
    ...
    */
    ...
    /*!
      ...
      A length \em n array containing a mean of the GMRF \c before correcting for constraints, if
      any.  If some of the elements of <em>\b x</em> are fixed, these fixed values are
      copied into the corresponding positions in \c mean, while the rest are the conditional mean
      from the conditional distribution given the fixed values.

      This array is just \ref GMRFLib_problem_tp::sub_mean copied into the correct locations, where
      the rest is fixed values.
    ...
    */
    ...
    /*!
      ...
      A length \em n array containing a mean of the GMRF \c after correcting for constraints, if
      any.  If some of the elements of <em>\b x</em> are fixed, these fixed values are copied into
      the corresponding positions in \c mean_constr, while the rest are samples (or configuration to
      be evalutated) from the conditional distribution given the fixed values.

      This array is just \ref GMRFLib_problem_tp::sub_mean_constr copied into the correct locations,
      where the rest is fixed values.
    ...
    */
    ...
    /*!
      ...
      The mapping between the elements of \a sub_sample, \a sub_mean, \a sub_mean_constr and \a
      sub_graph, and the real world. The correspondence is defined by \f$ \mbox{\small\tt
      sub\_sample}[i] \leftrightarrow \mbox{\small\tt sample} [\mbox{\small\tt map}[i]] \f$, and
      similar with the other quantities. \n\n
    */
    int *map;					  

    /*!
      \brief The log-density of sub_sample, corresponding to the elements 
      of <em>\b x</em> that are not fixed. 
    */
    double sub_logdens;				  
    ...
    */
    ...
    /*!
      ...
      The computed conditional mean (given the values of the fixed elements). This is the mean
      values of an \em unconstrained sample, that is, before taking into account any constraints.
    ...
    */
    ...
    /*!
      ...
      The mean of the variables corresponding to the sampled values in \a sample. This is the
      conditional mean of the sampled elements of <em>\b x</em>, given the values of the fixed
      elements and \em after taking any constraints into account.
    ...
    */
    ...
    /*!
      ...
      The subgraph defined on the nodes for which the elements of <em>\b x</em> are not fixed. This
      can be used to extract \f$ \mbox{\small\tt sub\_graph} \rightarrow \mbox{\small\tt n} \f$, the
      number of elements of the graph, equal to the length of the \a map-array. 
    */
...
struct GMRFLib_store_struct
{
    ...
    int store_problems;				  /* flag, the remaining args are only used if this
						   * is TRUE */
    int decision;

    GMRFLib_problem_tp *problem_old2new;
    GMRFLib_problem_tp *problem_new2old;
    double *old;				  /* old log-density */
    double *new;				  /* new log-density */
};
...
int GMRFLib_init_problem(GMRFLib_problem_tp **problem, double *x, double *b, double *c, double
			 *mean, GMRFLib_graph_tp *graph, GMRFLib_Qfunc_tp *Qfunc, char *Qfunc_args,
			 char *fixed_value, GMRFLib_constr_tp *constraint, unsigned int keep); 
int GMRFLib_init_problem_store(GMRFLib_problem_tp **problem, double *x, double *b, double *c,
				double *mean, GMRFLib_graph_tp *graph, GMRFLib_Qfunc_tp *Qfunc,
				char *Qfunc_args, char *fixed_value, GMRFLib_constr_tp *constr,
				unsigned int keep, GMRFLib_store_tp *store);  
...
int GMRFLib_recomp_constr(GMRFLib_constr_tp **new_constr, GMRFLib_constr_tp *constr,
			  double *x, double *b_add,
